...
<body>
    ...
    <header class="header">
            <nav class="navbar">
                ...
                <div class="nav-item">
                    ...
                    <div class="dropdown-container">
                        <a href="#">Categories <i class="ri-arrow-drop-down-fill"></i></a>
                        <ul class="dropdown">
        ...
        <section class="item-banner" style="height: 70vh ;background-image: url(images/login_background.jpg); position: bottom center;">
            <div class="txt">
            <h2 >Search by product name</h2>
            </div>
        </section> 
        <!--Item Section Design-->
        <section class="items" style="justify-content:left;">
            <div class="grid-container">
                <?php
                    if(isset($_GET['search'])){
                        ...
                        $sql = "select * from vegetables where name like '%$searchValue%'";
                        ...
                        if(mysqli_num_rows($result) > 0){
                            ...
                            $sql = "select * from fruits where name like '%$searchValue%'";
                            $result = mysqli_query($con,$sql);
                            if(mysqli_num_rows($result) > 0){
                                while($row = mysqli_fetch_assoc($result)){ 
                                ?>
                                ...
                                $sql = "select * from meats where name like '%$searchValue%'";
                                $result = mysqli_query($con,$sql);
                                if(mysqli_num_rows($result) > 0){
                                    while($row = mysqli_fetch_assoc($result)){ 
                                    ?>
                                        <div class="grid-item" style="padding:6%; height:350px">
                                                <img style="width: 120px;height: 120px;" src="<?php echo $row['img_url']?>"><br><br>
                                                <label class="name"><?php echo $row['name']?></label><br><br>
                                                <label class="prise">Rs <?php echo $row['price']?></label><br><br>
                                                <button class="cart-btn"><i class="ri-shopping-cart-2-fill"></i> Add to Cart</button>
